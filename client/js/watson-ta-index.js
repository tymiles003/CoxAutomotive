...
(function() {
  ...
  function loadTradeoffAnalytics(profile, themeName, callback, errCallback) {
    console.log("loadTradeoffAnalytics - INIT");


    <!-- // ****  TO BE CHANGED BASED ON CUSTOM SETTING  **** //-->
    <!-- // ****  SERVER API TO HANDLE WATSON SERVICE **** // -->

    taClient = new TradeoffAnalytics({
      dilemmaServiceUrl: '/api/watson-ta-graph',
      customCssUrl: 'http://ta-cdn.mybluemix.net/modmt/styles/' + themeName + '.css',
      profile: profile,
      errCallback: errCallback
    }, 'taWidgetContainer');
    console.log("INITIATING TradeoffAnalytics client COMPLETE - STARTING Process ...");
  ...
  function onPageLoad() {
    console.log("LOADING onPageLoad ...");
  ...
  function loadSelectedProblem() {
    ...
    <!-- // **** TO BE CHANGED BASED ON CUSTOM SETTING  **** //-->
    <!-- // **** ASSETS LOCATION **** // -->
    //var path = 'assets/' + $('.problems').val();

    if(sessionStorage.jsondata != undefined && sessionStorage.jsondata.trim() != "")  {
      $('.problemText').val(JSON.stringify(JSON.parse(sessionStorage.jsondata), null, 2)).change();
    }
    else {
      var path = 'assets/' + $('.problems').val();
  ...
  function onAnalyzeClick() {
    ...
    console.log("onAnalyzeClick - problemJson: " + JSON.stringify(problemJson));
    if (!problemJson)
      return;

    var featuresJson = getJsonFromElement($('.featuresText'));
    console.log("onAnalyzeClick - featuresJson: " + JSON.stringify(featuresJson));
    if (!featuresJson)
      return;
    ...
    if (themeChanged || profileChanged) {
      ...
      if (taClient) {
        destroyTradeoffAnalytcsWidget(function() {
          console.log("CALLING destroyTradeoffAnalytcsWidget ...");
