...
$(document).ready(function() {
    ...
    if (!$('body').hasClass('chrome')) {
        console.log("CHROME is NOT detected");
    ...
    speech.onend = function(data) {
        ...
        if($("#txtTtsQuestionIndex").val() != 7)    {
            $("#txtTtsAnswer").trigger("input");
            $("#txtTtsQuestionIndex").trigger("input");
            $(".capture-button").trigger("click");    
        }
    };

    speech.onresult = function(data) {
        console.log('demo.onresult(): ' + JSON.stringify(data));
        showResult(data);
    };
        ...
        if (data.results && data.results.length > 0) {
            ...
            if (data.results.length === 1 ) {
                ...
                if (data.results[0].final){
                    ...
                    $("#txtTtsAnswer").val(text);

                    //console.log("text 1 = " + $("#txtTtsAnswer").val() + "| txtTtsQuestionIndex = " + $("#txtTtsQuestionIndex").val());

                    if($("#txtTtsQuestionIndex").val() != 7)    // 7 is the vehicle selection question
    ...
    $('.send-api-audio1').click(function() {
        console.log("Send-API is clicked");
